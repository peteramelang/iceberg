...
for (const path of files) {
  ...
  const needsTldr = !fm.tldr || fm.tldr.toLowerCase().startsWith("pending");
  if (!needsTldr) continue;
  const card = {
    slug: fm.slug,
    title: fm.title,
    summary: fm.summary,
  ...
  writeFileSync(join(runsDir, `tldr-input-${fm.slug}.json`), JSON.stringify(card, null, 2) + "\n");
  written++;
}

console.log(`Wrote ${written} tldr input cards.`);
